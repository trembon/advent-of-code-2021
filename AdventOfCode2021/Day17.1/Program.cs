...
for (int x = xLower; x <= rangeX.Last(); x++)
{
    for (int y = rangeY.First(); y < Math.Abs(rangeY.First()); y++)
    {
        int xVel = x, yVel = y;
        int xPos = 0, yPos = 0;
        int highestYThisCombo = 0;

        while (yPos >= rangeY.First() && xPos <= rangeX.Last())
        {
            xPos += xVel; yPos += yVel;
            highestYThisCombo = Math.Max(highestYThisCombo, yPos); 
            if (xPos >= rangeX.First() && xPos <= rangeX.Last() && yPos <= rangeY.Last() && yPos >= rangeY.First())
            {
                highestYPosition = Math.Max(highestYPosition, highestYThisCombo);
            }
...
Console.WriteLine($"highest y: {highestYPosition}");