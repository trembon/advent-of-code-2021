...
{
    var cave = caves[id];
    if (cave.IsStart && path.Path.Contains(cave.Id))
        return;

    if (cave.IsSmall && path.Path.Contains(cave.Id))
    {
        var visitedSmall = path.Path.Where(x => char.IsLower(x[0])).GroupBy(x => x).Select(v => v.Count()).ToArray();
        if (visitedSmall.Any(x => x > 1))
            return;
    }
